...
/**
 * 该工具类用于生成多个用户(包括：电话、密码)，方便多用户高并发测试。功能如下：
 * 1. 自动创建多个用户,保存到用户表/seckill_user
 * 2. 模拟http请求,生成jmeter压测的脚本，即：config.txt(Jmeter中CSV数据文件设置中的文件名)
 */
//生成多用户工具类
//创建用户，并且去登录得到userTicket，得到的userTicket写入到config.txt 文件内
public class UserUtil {
    public static void create(int count) throws Exception {
        ...
        File file = new File("D:\\Jmeter\\apache-jmeter-5.4.3\\config.txt");
        if (file.exists()) {
            file.delete();
        }
